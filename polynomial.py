...
class Polynomial:
    ...
    def combine_like_terms(self) -> None:
        ...
        self.terms = [
            Term(sum([term.factor for term in self.terms if term.power == power]), power)
            for power in set(term.power for term in self.terms)
        ]

        self.terms = list(filter(lambda term: term.factor != 0 and not np.isclose(term.factor, 0, rtol=1e-05, atol=1e-08, equal_nan=False), self.terms))
    ...
    def value_at(self, x: float) -> float:
        return sum(
            term.insert(x) for term in self.terms
        )
